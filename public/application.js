...
var checkFull = function checkFull(element, index, array) {
  return element !== '';
};
...
var clearWin = function clearWin() {
  $('#alertWin').text('');
};
...
var newGame = function newGame(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.createGame(data).then(ui.startNewGame).catch(ui.startNewGameFailure);
};

var gamePlayed = function gamePlayed() {
  api.gamesPlayed().then(ui.getGamesPlayed).catch(ui.getGamesFailure);
};
